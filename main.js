...
const phoneNumber = await question(color('masukin no wa pake awalan 628:\n', 'red'));
const code = await Skyzo.requestPairingCode(phoneNumber.trim())
console.log(`${chalk.redBright('masukin kode ini di WhatsApp')} : ${code}`)
}
store.bind(Skyzo.ev)
...
Skyzo.sendMessage("${global.owner}@s.whatsapp.net", {text: "bot berhasil terhubung"})
...
if (ff.isGroup == false) {
if (ff.status == "offer") {
let sendcall = await Skyzo.sendMessage(ff.from, {text: `@${ff.from.split("@")[0]} Maaf Kamu Akan Saya Block Karna Ownerbot Menyalakan Fitur *Anticall*\nJika Tidak Sengaja Segera Hubungi Owner Untuk Membuka Blokiran Ini`, contextInfo: {mentionedJid: [ff.from], externalAdReply: {thumbnailUrl: "https://t1.pixhost.to/thumbs/5378/593401890_20250424_141012.jpg", title: "CALL DETECTED", previewType: "PHOTO"}}}, {quoted: null})
...
ppuser = 'https://t1.pixhost.to/thumbs/5378/593401890_20250424_141012.jpg'
}
if (anu.action == 'add') {
